...
class DatabaseManager:
    ...
    def get_last_trade_time_for_market(self, market_id: str):
        """Returns last trade timestamp (ISO str) for this market_id, or None."""
        if not market_id:
            return None
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT timestamp FROM trades WHERE market_id = ? ORDER BY id DESC LIMIT 1",
                    (str(market_id),),
                )
                row = cursor.fetchone()
